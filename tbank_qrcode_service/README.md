...
Требуют запущенный сервер. По умолчанию подключаются к `http://localhost:8080`.

```bash
# Указать адрес сервера через переменную окружения
TEST_BASE_URL=http://208.92.226.223:8080 mvn test -Dgroups=integration
```
...
k6 run performance-tests/load-test-tbank.js
```

## Примечание об исходном коде

Исходный код восстановлен из JAR-файла (`payment-api-3.0.jar`) с помощью декомпилятора CFR.
Код полностью функционален, но может содержать артефакты декомпиляции.
Секреты (Tinkoff API keys) вынесены в `application.properties` и не содержатся в коде.
